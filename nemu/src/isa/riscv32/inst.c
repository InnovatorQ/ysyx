...
__instpat_end_: ; }
  ...
  */
  ...
  INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu  , R, R(rd) = ((uint64_t)src1 * (uint64_t)src2) >> 32); //增加mulhu src1和src2视为无符号数拓展至64位
  ...
  INSTPAT("??????? ????? ????? 010 ????? 00100 11", slti   , I, R(rd) = (sword_t)src1 < (sword_t)imm ? 1 : 0); //增加slti
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , I, R(rd) = (sword_t)src1 < (sword_t)imm ? 1 : 0); //增加sltiu
  INSTPAT("0000000 ????? ????? 011 ????? 01100 11", sltu   , R, R(rd) = (word_t)src1 < (word_t)src2 ? 1 : 0); //增加sltu !!!就算按有符号实现也会PASS
  ...
  INSTPAT("0011000 00010 00000 000 00000 11100 11", mret   , N, s->dnpc = mepc; mstatus = (mstatus & ~0x8) | ((mstatus & 0x80) >> 7); );
