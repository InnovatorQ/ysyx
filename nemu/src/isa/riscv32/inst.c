...
__instpat_end_: ; }
  ...
  */
  ...
  INSTPAT("??????? ????? ????? 000 ????? 00100 11", addi   , I, R(rd) = src1 + imm);  // 增加addi指令模式
  INSTPAT("??????? ????? ????? ??? ????? 01101 11", lui    , U, R(rd) = imm); // 增加lui指令模式
  INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu    , I, R(rd) = Mr(src1 + imm, 1));
  INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw     , I, R(rd) = Mr(src1 + imm, 4)); //增加lw
  INSTPAT("??????? ????? ????? 000 ????? 01000 11", sb     , S, Mw(src1 + imm, 1, src2));
  INSTPAT("??????? ????? ????? 010 ????? 01000 11", sw     , S, Mw(src1 + imm, 4, src2)); //增加sw
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(rd) = s->pc + 4; s->dnpc = s->pc + imm);  //增加jal
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, { word_t t = s->pc + 4; s->dnpc = (src1 + imm) & ~1; R(rd) = t; }); //增加jalr
  INSTPAT("0000000 ????? ????? 000 ????? 01100 11", add    , N, R(rd) = src1 + src2); //增加add
  INSTPAT("0100000 ????? ????? 000 ????? 01100 11", sub    , N, R(rd) = src1 - src2); //增加sub
