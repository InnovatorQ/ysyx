...
__instpat_end_: ; }
  ...
  */
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(rd) = s->pc + 4; s->dnpc = s->pc + imm);  //增加jal work!
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, { word_t t = s->pc + 4; s->dnpc = (src1 + imm) & ~1; R(rd) = t; }); //增加jalr work!
  ...
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , R, R(rd) = ((int64_t)(sword_t)src1 * (int64_t)(sword_t)src2) >> 32); //增加mulh !!!!
